                    ...
                    @foreach($metodosPago as $mp)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-700">
                                {{ Str::limit($mp->descripcion, 50, '...') }}
                            </td>
                            <td class="px-6 py-4 text-sm">
                                <div class="flex gap-2">
                                    <a href="{{ route('metodos_pago.show', $mp->id_metodo) }}"
                                       class="px-2.5 py-1 bg-blue-100 text-blue-700 rounded-md text-xs font-medium hover:bg-blue-200 transition">
                                        Ver
                                    </a>
                                    <a href="{{ route('metodos_pago.edit', $mp->id_metodo) }}"
                                       class="px-2.5 py-1 bg-yellow-100 text-yellow-700 rounded-md text-xs font-medium hover:bg-yellow-200 transition">
                                        Editar
                                    </a>
                                    <form action="{{ route('metodos_pago.destroy', $mp->id_metodo) }}" method="POST" onsubmit="return confirm('¿Eliminar este método de pago?')">
                                        @csrf @method('DELETE')
