...
<div class="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-sm border border-gray-200">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Editar Compra</h1>
    ...
    <form action="{{ route('compras.update', $compra->id_compra) }}" method="POST">
            ...
            >
                ...
                @foreach($proveedores as $proveedor)
                    <option value="{{ $proveedor->id_proveedor }}" 
                        {{ (old('proveedor_id', $compra->proveedor_id) == $proveedor->id_proveedor) ? 'selected' : '' }}>
                        {{ $proveedor->nombre }}
                    </option>
                @endforeach
            </select>
            ...
            >
                ...
                @foreach($usuarios as $usuario)
                    <option value="{{ $usuario->id_usuario }}" 
                        {{ (old('usuario_id', $compra->usuario_id) == $usuario->id_usuario) ? 'selected' : '' }}>
                        {{ $usuario->nombres }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-5">
            ...
            <input
                ...
                name="total_compra"
                id="total_compra"
                value="{{ old('total_compra', $compra->total_compra) }}"
                class="w-full px-3 py-2.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-marron-oscuro focus:border-marron-oscuro"
                min="0"
                required
            >
        ...
        <div class="mb-5">
            ...
            <input
                ...
                name="metodo_pago"
                id="metodo_pago"
                value="{{ old('metodo_pago', $compra->metodo_pago) }}"
                class="w-full px-3 py-2.5 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-marron-oscuro focus:border-marron-oscuro"
